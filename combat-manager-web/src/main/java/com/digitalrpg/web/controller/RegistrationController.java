...
public class RegistrationController {
	...
	public ModelAndView getHeading(HttpServletRequest request, HttpServletResponse response) {
		...
		if(savedRequest != null) {
			...
			if(messages != null && messages.length > 0) {
				MessageVO message = messageService.getMessage(Long.valueOf(messages[0]));
				if(message != null && message instanceof InviteMessageVO) {
					modelMap.put("message", 
						String.format("<p><strong>%s</strong> has invited you to join the campaign \"%s\".</p> <small>Use the form below to sign up or login and claim your spot at the table.</small>",
								message.getFrom().getName(), ((InviteMessageVO)message).getCampaignName()));
				}
			}
		} 
	...
	public ModelAndView registerUser(@Valid @ModelAttribute("user") UserRegistrationVO userRegistrationVO, BindingResult bindingResult, HttpServletRequest request, final RedirectAttributes redirectAttributes, HttpServletResponse response) throws ServletException, IOException {
		String remoteAddr = request.getRemoteAddr();
		ReCaptchaImpl reCaptcha = new ReCaptchaImpl();
		reCaptcha.setPrivateKey("6LfGTOgSAAAAABbQhBkr36lp24ucA_NVEIJQKBDq");
		ReCaptchaResponse answer = reCaptcha.checkAnswer(remoteAddr, userRegistrationVO.getRecaptcha_challenge_field(), userRegistrationVO.getRecaptcha_response_field());
		if(!answer.isValid()) {
		...
		User user = registrationService.registerUser(userRegistrationVO.getUsername(), userRegistrationVO.getEmail(), userRegistrationVO.getPassword(), contextPath);
	...
	private Authentication authenticateUserAndSetSession(UserRegistrationVO userRegistrationVO,
	        HttpServletRequest request)
	{
	    UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(
	            userRegistrationVO.getUsername(), userRegistrationVO.getPassword());
