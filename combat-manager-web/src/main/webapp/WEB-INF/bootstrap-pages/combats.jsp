		...
		<div class="row">
			<div class="hidden-xs col-sm-3 content-block">
				<div class="cbp-spmenu" id="accordion">
					<h4 class="inverse">My Active Combats</h4>
					<div class="">
						<div class="">
							<h5>
								<a data-toggle="collapse" data-parent="#accordion"
									href="#collapseActivePlayer"> I'm a player in <span
									id="collapseActivePlayerIcon"
									class="pull-right glyphicon glyphicon-chevron-down"></span>
								</a>
							</h5>
						</div>
						<div id="collapseActivePlayer" class="collapse">
							<div class="scroll_list_150">
								<c:forEach items="${activeCombats}" var="combat">
									<c:if test="${combat.campaign.gameMaster.name != user.name }">
										<c:url value="/combats/${combat.id }/show" var="url"></c:url>
										<a class="campaign_list_item short" id="combat_${combat.id }"
											href="${url }">${combat.name}</a>
									</c:if>
								</c:forEach>
							</div>
						</div>
						<div class="">
							<h5>
								<a data-toggle="collapse" data-parent="#accordion"
									href="#collapseActiveGM"> I'm a GM of <span
									id="collapseActiveGMIcon"
									class="pull-right glyphicon glyphicon-chevron-down"></span>
								</a>
							</h5>
						</div>
						<div id="collapseActiveGM" class="collapse">
							<div class="scroll_list_150">
								<c:forEach items="${activeCombats}" var="combat">
									<c:if test="${combat.campaign.gameMaster.name == user.name }">
										<c:url value="/combats/${combat.id }/show" var="url"></c:url>
					...
					<h4 class="inverse">Future Combats</h4>
					<div class="">
						<div class="">
							<h5>
								<a data-toggle="collapse" data-parent="#accordion"
									href="#collapseFuturePlayer"> I'm a player in <span
									id="collapseFuturePlayerIcon"
									class="pull-right glyphicon glyphicon-chevron-down"></span>
								</a>
							</h5>
						</div>
						<div id="collapseFuturePlayer" class="collapse">
							<div class="scroll_list_150">
								<c:forEach items="${futureCombats}" var="combat">
									<c:if test="${combat.campaign.gameMaster.name != user.name }">
										<c:url value="/combats/${combat.id }/show" var="url"></c:url>
										<a class="campaign_list_item short" id="combat_${combat.id }"
											href="${url }">${combat.name}</a>
						...
						<div class="">
							<h5>
								<a data-toggle="collapse" data-parent="#accordion"
									href="#collapseFutureGM"> I'm a GM of <span
									id="collapseFutureGMIcon"
									class="pull-right glyphicon glyphicon-chevron-down"></span>
								</a>
							</h5>
						</div>
						<div id="collapseFutureGM" class="collapse">
							<div class="scroll_list_150">
								<c:forEach items="${futureCombats}" var="combat">
									<c:if test="${combat.campaign.gameMaster.name == user.name }">
										<c:url value="/combats/${combat.id }/show" var="url"></c:url>
					...
					<h4 class="inverse">Historic Combats</h4>
					<div class="">
						<div class="">
							<h5>
								<a data-toggle="collapse" data-parent="#accordion"
									href="#collapseHistoricPlayer"> I'm a player in <span
									id="collapseHistoricPlayerIcon"
									class="pull-right glyphicon glyphicon-chevron-down"></span>
								</a>
							</h5>
						</div>
						<div id="collapseHistoricPlayer" class="collapse">
							<div class="scroll_list_150">
								<c:forEach items="${historicCombats}" var="combat">
									<c:if test="${combat.campaign.gameMaster.name != user.name }">
										<c:url value="/combats/${combat.id }/show" var="url"></c:url>
										<a class="campaign_list_item short" id="combat_${combat.id }"
											href="${url }">${combat.name}</a>
						...
						<div class="">
							<h5>
								<a data-toggle="collapse" data-parent="#accordion"
									href="#collapseHistoricGM"> I'm a GM of <span
									id="collapseHistoricGMIcon"
									class="pull-right glyphicon glyphicon-chevron-down"></span>
								</a>
							</h5>
						</div>
						<div id="collapseHistoricGM" class="collapse">
							<div class="scroll_list_150">
								<c:forEach items="${historicCombats}" var="combat">
									<c:if test="${combat.campaign.gameMaster.name == user.name }">
										<c:url value="/combats/${combat.id }/show" var="url"></c:url>
			...
			<div class="col-xs-12 col-sm-7">
				...
				<c:if test="${createCombatVO != null }">
					<div id="combat_create"
						...
						<form:form action="${createCombatUrl }"
							modelAttribute="createCombatVO" id="create-combat" method="POST">
							...
							<spring:bind path="name">
								<div class="form-group ${status.error? 'has-error' :'' }">
									<label class="control-label" for="name">Name <form:errors
										...
										required="true" value="${createCombatVO.name }" />
								</div>
							</spring:bind>
							...
							<spring:bind path="description">
								<div class="form-group ${status.error? 'has-error' :'' }">
									...
									<textarea class="form-control" rows="4" id="description"
										name="description">${createCombatVO.description }</textarea>
									<form:errors element="div" path="description" />
								</div>
							</spring:bind>

							<div class="form-group">
								<input type="hidden" name="players" id="selectedPlayers"
									value="${createCombatVO.players }" />
								<div class="row">
									<div class="col-xs-6">
															...
															<li class="dd-item dd3-item" data-id="${character.id}">
																...
																<div class="dd3-content">
																	<p class="overflown tooltipable">
																		<c:if
																			test="${character.character.characterType == 'NPC' }">
																			<!-- GM Char, can be hidden -->
																			<span class="pull-right"> <input
																				class="iconToggle" type="checkbox"
																				name="extraInfo[${character.id }].hidden" />
																			</span>
																		</c:if>
												...
												<c:otherwise>
													<c:forEach var="character" items="${currentCharacters}">
														<li class="dd-item dd3-item"
															data-id="${character.character.id}">
															<div class="dd-handle dd3-handle">Drag</div>
															<div class="dd3-content">

																<p class="overflown tooltipable">
																	<c:if
																		test="${character.character.character.owner.name == user.name }">
																		<!-- GM Char, can be hidden -->
																		<span class="pull-right"> <input
																			class="iconToggle" type="checkbox"
																			name="extraInfo[${character.character.id }].hidden" />
																		</span>
																	</c:if>
							...
							<input type="submit"
								value="${empty createCombatVO.id ? 'Create' : 'Save'}"
				...
				<div id="combat_view"
					...
					<div class="row">
						<div class="col-xs-8 left-pane ">
							<p>
								<c:if
									test="${combat.campaign.gameMaster.name == user.username }">
									<div class="dropdown pull-right">
										...
										<ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
											<c:if test="${combat.state == 'STAGING' }">
												<li><c:url var="url"
														value="/combats/${combat.id }/edit"></c:url> <a class=""
													href="${url }"><i class="fa fa-pencil-square-o fa-fw">
															<!--  -->
													</i>&#160; Edit Combat</a></li>
												<li><c:url var="url"
														value="/combats/${combat.id }/accept"></c:url> <a class=""
													href="${url }"><i class="fa fa-check fa-fw">
															<!--  -->
													</i>&#160; Mark as Ready</a></li>
											</c:if>
											<c:if test="${combat.state == 'READY' }">
												<li><c:url var="url"
														value="/combats/${combat.id }/start"></c:url> <a class=""
													href="${url }"><i class="fa fa-play fa-fw">
															<!--  -->
													</i>&#160; Start Combat</a></li>
											</c:if>
											<c:if
												test="${combat.state == 'STAGING' || combat.state == 'READY'}">
												<li><a href="#" data-toggle="modal"
													data-target="#confirm-delete"><i
														class="fa fa-trash-o fa-fw">
															<!--  -->
													</i>&#160; Delete Combat</a></li>
											</c:if>
											<c:if test="${combat.state == 'FINISHED'}">
												<li><a href="#" data-toggle="modal"
													data-target="#view-logs"><i
							...
							<div class="form-group">
								...
								<p class="form-control-static">
									...
									<a href="${url }"> ${combat.campaign.name } </a>
								</p>
							</div>
							<div id="combat_description" class="scroll_description">
								${combat.description }</div>
						</div>
								...
								<div class="">
									<div class="">
										<h5>
											<a data-toggle="collapse" data-parent="#rightBar"
												href="#collapseMembers"> Combatants <span
									...
									<div id="collapseMembers" class="collapse in">
										<div class="scroll_list_150 nice_list">
											<ul>
												<c:forEach items="${combat.combatCharacters}"
													var="combatant">
													<c:if
														test="${combat.campaign.gameMaster.name == user.username || !combatant.hidden}">
