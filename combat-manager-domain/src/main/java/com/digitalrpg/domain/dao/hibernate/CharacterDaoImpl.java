...
public class CharacterDaoImpl extends HibernateDao implements CharacterDao {
    ...
    public SystemCharacter get(Long id) {
        List<SystemCharacter> list =
                sessionFactory.getCurrentSession()
                        .createQuery("from SystemCharacter sc where sc.id = ?").setParameter(0, id)
                        .list();
        if (list.size() == 1) {
    ...
    public Boolean hasPlayerCharacter(Campaign campaign, User user) {
        List<SystemCharacter> characters =
                sessionFactory
                        ...
                        .setParameter("user", user).setParameter("campaign", campaign).list();
    ...
    public void deleteSystemCharacter(Long id) {
        SystemCharacter systemCharacter = this.get(id);
        this.sessionFactory.getCurrentSession().delete(systemCharacter);
