...
public class CampaignDaoImpl extends HibernateDao implements CampaignDao {
    ...
    public List<Campaign> getCampaignsForUser(User user) {
        // user = (User) this.sessionFactory.getCurrentSession().merge(user);
        List<Campaign> campaignsAsPlayer =
                sessionFactory.getCurrentSession().createQuery("select distinct c from Campaign c where :user in elements(c.members)")
                        .setParameter("user", user).list();
        List<Campaign> campaignsAsGameMaster =
                sessionFactory.getCurrentSession().createQuery("select c from Campaign c where c.gameMaster = :user")
                        .setParameter("user", user).list();
    ...
    public Campaign get(Long id) {
        List<Campaign> campaigns = sessionFactory.getCurrentSession().createQuery("from Campaign where id = ?").setParameter(0, id).list();
        if (campaigns.size() == 1) {
    ...
    public Collection<Campaign> search(String searchString, int offset, int limit) {
        ...
        List list =
                sessionFactory
                        .getCurrentSession()
                        .createQuery(
                                "from Campaign c where c.isPublic = true and (upper(c.name) like :searchString or upper(c.description) like :searchString) order by c.name")
                        .setParameter("searchString", "%" + searchString.toUpperCase() + "%").setFirstResult(offset).setMaxResults(limit)
                        .list();
