...
public class UserDaoImpl extends HibernateDao implements UserDao {
    ...
    public void activateUser(String name, String activationToken) {
        List<User> list =
                sessionFactory.getCurrentSession().createQuery("from User where name = ?")
    ...
    public User findByMail(String emailTo) {
        List<User> list =
                sessionFactory.getCurrentSession().createQuery("from User where email = ?")
                        .setParameter(0, emailTo).list();
        if (list.size() > 0) {
